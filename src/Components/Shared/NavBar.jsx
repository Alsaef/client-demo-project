    ...
    <header className="w-full font-sans bg-white shadow-sm border-b border-gray-100">
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <button 
          ...
          className="lg:hidden text-gray-700 hover:text-[#49A760] focus:outline-none p-1 transition-colors"
        ...
        <div className="hidden lg:flex flex-1 max-w-xl mx-8 relative">
          <input
            ...
            className="w-full bg-gray-100 text-gray-700 pl-4 pr-10 py-2 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-[#49A760] border border-transparent"
          />
          <button className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-[#49A760] transition-colors">
        ...
        <div className="hidden lg:flex items-center space-x-6 text-gray-700 text-xs font-medium">
          <Link to="/track-order" className="flex flex-col items-center hover:text-[#49A760] transition-colors gap-1">
            <FiMapPin className="w-5 h-5" />
            <span>Track Order</span>
          </Link>
          <Link to="/signin" className="flex flex-col items-center hover:text-[#49A760] transition-colors gap-1">
            <FiUser className="w-5 h-5" />
            <span>Sign In</span>
          </Link>
          <Link to="/wishlist" className="flex flex-col items-center hover:text-[#49A760] transition-colors gap-1">
            <FiHeart className="w-5 h-5" />
            <span>Wishlist</span>
          </Link>
          <label htmlFor="cart-drawer" className="flex flex-col items-center hover:text-[#49A760] transition-colors gap-1 relative cursor-pointer">
            <div className="relative">
              <FiShoppingCart className="w-5 h-5" />
              <span className="absolute -top-2 -right-2 bg-[#49A760] text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
                0
              </span>
            </div>
            <span>Cart</span>
          ...
          {/* More Dropdown */}
          <div className="relative group flex flex-col items-center cursor-pointer hover:text-[#49A760] transition-colors gap-1">
            ...
            <div className="absolute right-0 top-full pt-2 hidden group-hover:block z-50 min-w-[160px]">
              <ul className="bg-white text-gray-800 rounded-md shadow-lg py-2 border border-gray-100 text-xs font-normal">
                <li>
                  <Link to="/about" className="flex items-center gap-2 px-4 py-2 hover:bg-green-50 hover:text-[#49A760] transition-colors">
                    <FiInfo className="w-4 h-4" /> About Us
                  </Link>
                </li>
                <li>
                  <a href="tel:+8801700000000" className="flex items-center gap-2 px-4 py-2 hover:bg-green-50 hover:text-[#49A760] transition-colors">
                    <FiPhone className="w-4 h-4" /> Call Us
                  </a>
                </li>
                <li>
                  <a href="https://wa.me/8801700000000" target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 hover:bg-green-50 hover:text-[#49A760] transition-colors">
        ...
        <div className="lg:hidden flex items-center">
          <label htmlFor="cart-drawer" className="relative p-2 text-gray-700 cursor-pointer">
            <FiShoppingCart className="w-6 h-6" />
            <span className="absolute top-1 right-1 bg-[#49A760] text-white text-xs w-4 h-4 rounded-full flex items-center justify-center font-bold">
              0
            </span>
          </label>
        </div>
      ...
      <div className="lg:hidden px-4 pb-3">
        <div className="relative w-full">
          <input
            type="text"
            placeholder="Search in..."
            className="w-full bg-gray-100 text-gray-700 pl-4 pr-10 py-2 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-[#49A760]"
          />
          <button className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-[#49A760]">
          ...
          <ul className="flex items-center justify-between text-xs font-medium py-3">
            {navCategories.map((cat, idx) => (
              <li key={idx} className="relative group">
                <Link
                  to={cat.path}
                  className="flex items-center gap-1 hover:text-[#49A760] transition-colors py-1"
                >
                  {cat.name}
                    ...
                    <ul className="bg-white text-gray-800 rounded-md shadow-lg py-2 border border-gray-100">
                      {cat.subItems.map((sub, sIdx) => (
                        <li key={sIdx}>
                          <Link
                            to={sub.path}
                            className="block px-4 py-2 hover:bg-green-50 hover:text-[#49A760] text-xs transition-colors"
                          >
                            {sub.name}
                          </Link>
                        </li>
          ...
          <div className="relative bg-white w-[280px] max-w-full h-full shadow-xl flex flex-col z-10 overflow-y-auto">
            ...
            <div className="p-4 bg-white border-b border-gray-100 flex items-center justify-between">
              <Link 
                to="/signin" 
                onClick={() => setIsMobileMenuOpen(false)}
                className="flex items-center gap-3 bg-[#49A760] text-white w-full p-3 rounded-lg shadow-sm"
              >
                ...
                <div>
                  ...
                  <div className="text-xs text-green-100">Signin</div>
                </div>
              </Link>
              <button 
                onClick={() => setIsMobileMenuOpen(false)}
                  ...
                  <li key={idx} className="border-b border-gray-50 last:border-0">
                    <div className="flex items-center justify-between py-2.5 px-2">
                      <Link 
                        to={cat.path} 
                        onClick={() => setIsMobileMenuOpen(false)}
                        className="text-gray-700 text-sm font-medium hover:text-[#49A760] transition-colors"
                      >
                        {cat.name}
                      </Link>
                        ...
                        {cat.subItems.map((sub, sIdx) => (
                          <li key={sIdx}>
                            <Link
                              to={sub.path}
                              onClick={() => setIsMobileMenuOpen(false)}
                              className="block py-1.5 px-3 text-xs text-gray-600 hover:text-[#49A760] transition-colors"
                            >
                              {sub.name}
                            </Link>
                          </li>
                ...
                <ul className="space-y-1">
                  <li>
                    <Link 
                      to="/about" 
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center gap-2 py-2 px-2 text-sm text-gray-700 hover:text-[#49A760] transition-colors"
                    >
